...
2. If the requested release is indexed, follow this ordered search decision:
   ...
   3. After a baseline or retry produces a result set, classify the request:
      ...
      For API questions involving multiple methods, first establish the object with an object-focused probe, then run a method-focused probe for each important method if adjacent objects share similar names. For example, `GlideDate` and `GlideDateTime` can cross-rank in a broad query; require the returned `objectName` and `methodName` metadata to match before using the result.
