...
export class DocumentationSearchDatabase {
  ...
  private migrate(): void {
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_documents_filters ON documents(release, doc_type, publication, chunk_type);
      CREATE VIRTUAL TABLE IF NOT EXISTS documents_fts USING fts5(title, heading, content);
    `);
    ...
    this.db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS document_vectors USING vec0(embedding float[${this.dimensions}] distance_metric=cosine)`);
  ...
  replaceSources(release: string, sources: Array<{ path: string; blobSha: string; contentHash: string; chunks: DocumentChunk[]; embeddings: Float32Array[] }>, deletedPaths: string[]): void {
    ...
    const insert = this.db.transaction(() => {
      ...
      const insertFts = this.db.prepare("INSERT INTO documents_fts(rowid, title, heading, content) VALUES (?, ?, ?, ?)");
      const insertVector = this.db.prepare("INSERT INTO document_vectors(rowid, embedding) VALUES (?, ?)");
      ...
      for (const source of sources) {
        ...
        source.chunks.forEach((chunk, index) => {
          ...
          insertFts.run(id, chunk.title, chunk.heading, chunk.content);
          insertVector.run(id, vectorBuffer(source.embeddings[index]!));
        });
  ...
  search(query: string, embedding: Float32Array, filters: SearchFilters, limit = 10, threshold = 0.3, deduplicateReleases = false): SearchResult[] {
    ...
    const semantic = this.db.prepare(`
      ...
      WHERE v.embedding MATCH ? AND k = ?${filtersSql}
    ...
    const keyword = keywordExpression ? this.db.prepare(`
      SELECT d.*, f.rank, vec_distance_cosine(v.embedding, ?) AS distance
    ...
    keyword.forEach((row, index) => {
      ...
      scores.set(row.id, { row, score: (current?.score ?? 0) + 1 / (60 + index + 1), similarity: current?.similarity ?? similarity });
    });
    const multipliers: Partial<Record<DocumentChunk["chunkType"], number>> = { overview: 1, section: 0.95, definition: 0.85, method: 0.8, endpoint: 0.8, parameter: 0.5, returns: 0.5, example: 0.45 };
    const ranked = [...scores.values()]
      .map(({ row, score, similarity }) => {
        ...
        return { ...chunk, metadata, content: row.content.slice(0, 1000), score: score * (multipliers[row.chunk_type] ?? 0.9), similarity };
      })
      .sort((left, right) => right.score - left.score);
    if (!deduplicateReleases) return ranked.slice(0, boundedLimit);
    const seen = new Set<string>();
    return ranked.filter((result) => {
      const key = `${result.sourcePath}\u0000${result.chunkIndex}`;
